...
public partial class SpawnSystem : SystemBase {
        ...
        foreach (EntityBuildData plexusBuildData in SpawnQueue.Instance.PlexusBuildDataQueue) {
            ...
            for (int i = 0; i < plexusBuildData.Mesh.triangles.Length - 2; i = i + 3) {
                ...
                if (connections.Contains(new Connection { Id1 = pos1Id, Id2 = pos2Id }) == false && connections.Contains(new Connection { Id1 = pos2Id, Id2 = pos1Id }) == false) {
                    ...
                    AddPlexusLine(ref ecb,wireframePlexusEdgeSpawnData.WireframePlexusEdgeEntityPrefab, pos1Id, pos2Id, math.distance(plexusBuildData.Mesh.vertices[pos1Id], plexusBuildData.Mesh.vertices[pos2Id]), wireframePlexusObjectEntity);
                }
                if (connections.Contains(new Connection { Id1 = pos2Id, Id2 = pos3Id }) == false && connections.Contains(new Connection { Id1 = pos3Id, Id2 = pos2Id }) == false) {
                    ...
                    AddPlexusLine(ref ecb, wireframePlexusEdgeSpawnData.WireframePlexusEdgeEntityPrefab, pos2Id, pos3Id, math.distance(plexusBuildData.Mesh.vertices[pos2Id], plexusBuildData.Mesh.vertices[pos3Id]), wireframePlexusObjectEntity);
                }
    ...
    private void AddPlexusLine(ref EntityCommandBuffer ecb, Entity plexusLinePrefabEntity, int pointId1, int pointId2, float plexusLineLength, Entity parentEntity) {
        Entity plexusLineEntity = ecb.Instantiate(plexusLinePrefabEntity);
        ecb.AddSharedComponent(plexusLineEntity, new PlexusObjectIdData { ObjectId = plexusObjectId });
        ecb.SetComponent(plexusLineEntity, new EdgeData { Vertex1Index = pointId1, Vertex2Index = pointId2, Length = plexusLineLength });
        ecb.AddComponent(plexusLineEntity, new Parent { Value = parentEntity });
    }
